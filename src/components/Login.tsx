...
const Login = () => {
  ...
  const authorization = async () => {
    const data = {email: login, password: password};
      const authorizationData = await http.post(`https://reqres.in/api/${
        isLogin ? 'login' : 'registration'}`, data);
  ...
  return (
    <div className='row'>
      ...
      <Link to={isLogin ? 'registration' : 'login'}>
        {isLogin ? 'Do not have a account? Registration!' : 'Have a account? Login'}
      </Link>
      <div className="align-items-center">
        <button className="btn btn-primary col-3 m-3"
